...
const normalizeSchedules = (value: unknown): { time: string; capacity: number }[] => {
  ...
  return items.map((item) => {
    if (typeof item === 'string') {
      ...
      try {
        ...
        if (typeof parsed === 'object' && parsed !== null) {
          return {
            time: normalizeTime(String((parsed as { time?: unknown }).time || '')),
            capacity: Number((parsed as { capacity?: unknown }).capacity) || defaultCapacity,
          };
        }
      } catch {
    ...
    if (typeof item === 'object' && item !== null) {
      const schedule = item as { time?: unknown; capacity?: unknown };
      return {
        time: normalizeTime(String(schedule.time || '')),
        capacity: Number(schedule.capacity) || defaultCapacity,
      };
    }
    return null;
...
export async function GET() {
  ...
  const { data: configData, error: configError } = await supabase
    .from('form_config')
    .select('schedules, details, bank_name, account_number, depositor, price, price_2_people, background_image, email_notification_enabled, admin_email, updated_at')
  ...
  return NextResponse.json({
    ...
    emailNotificationEnabled: record?.email_notification_enabled ?? false,
...
export async function PUT(req: NextRequest) {
  ...
  const { schedules, details, bankName, accountNumber, depositor, price, price2, backgroundImage, emailNotificationEnabled, adminEmail } = await req.json();
  ...
  const payload = {
    ...
    email_notification_enabled: emailNotificationEnabled ?? false,
    admin_email: adminEmail ? String(adminEmail).trim() : '',
