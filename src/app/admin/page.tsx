...
type ApiSchedule = string | { time?: string; capacity?: number };
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'settings' | 'notifications'>('orders');
  ...
  const normalizeSchedule = (schedule: ApiSchedule): ScheduleConfig | null => {
    if (typeof schedule === 'string') {
      return { time: schedule, capacity: 100 };
    }

    if (schedule && typeof schedule === 'object' && typeof schedule.time === 'string') {
      return {
        time: schedule.time,
        capacity: typeof schedule.capacity === 'number' ? schedule.capacity : 100,
      };
    }
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data.schedules)) {
        normalizedSchedules = data.schedules
          .map(normalizeSchedule)
          .filter((schedule): schedule is ScheduleConfig => Boolean(schedule?.time));
      } else {
      ...
      setConfig({
        ...
        price: data.price !== undefined ? String(data.price) : defaultFormConfig.price,
        price2: data.price2 !== undefined ? String(data.price2) : (defaultFormConfig.price2 || '150000'),
    ...
    <main className={styles.page}>
      ...
      {activeTab === 'notifications' && (
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            <h2>알림 설정</h2>
            <p className={styles.muted}>클래스 신청이 들어오면 이메일로 알림을 받을 수 있습니다.</p>
          </div>
          ...
          <div className={styles.notificationCard}>
            <div className={styles.toggleRow}>
              <div className={styles.toggleInfo}>
                <h3>이메일 알림</h3>
                <p>새로운 클래스 신청이 접수되면 이메일로 알림을 받습니다.</p>
              </div>
              <label className={styles.toggle}>
                <input
                  type="checkbox"
                  checked={config.emailNotificationEnabled}
                  onChange={(e) => setConfig((prev) => ({ ...prev, emailNotificationEnabled: e.target.checked }))}
                />
                <span className={styles.toggleSlider}></span>
              </label>
            </div>

            {config.emailNotificationEnabled && (
          ...
          <div className={styles.infoBox}>
            ...
            <ul>
              <li>이메일 알림을 사용하려면 Resend API 키가 환경변수에 설정되어 있어야 합니다.</li>
              <li>환경변수: <code>RESEND_API_KEY</code></li>
              <li>Resend 가입 및 API 키 발급: <a href="https://resend.com" target="_blank" rel="noopener noreferrer">resend.com</a></li>
            </ul>
          </div>
